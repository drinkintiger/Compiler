...
struct ExprRes * doAdd(struct ExprRes * Res1, struct ExprRes * Res2) {
	...
	AppendSeq(Res1->Instrs,
			GenInstr(NULL, "add", TmpRegName(reg), TmpRegName(Res1->Reg),
					TmpRegName(Res2->Reg)));
...
ExprRes * doSubtr(ExprRes * Res1, ExprRes * Res2) {
	...
	AppendSeq(Res1->Instrs, Res2->Instrs);
	AppendSeq(Res1->Instrs,
			GenInstr(NULL, "sub", TmpRegName(reg), TmpRegName(Res1->Reg),
			  TmpRegName(Res2->Reg)));
	ReleaseTmpReg(Res1->Reg);
	ReleaseTmpReg(Res2->Reg);
	Res1->Reg = reg;
...
ExprRes * doDiv(ExprRes * Res1, ExprRes * Res2){
	...
	reg = AvailTmpReg();
	AppendSeq(Res1->Instrs, Res2->Instrs);
	AppendSeq(Res1->Instrs, GenInstr(NULL, "div", TmpRegName(reg), TmpRegName(Res1->Reg),
			TmpRegName(Res2->Reg)));
	ReleaseTmpReg(Res1->Reg);
	ReleaseTmpReg(Res2->Reg);
	Res1->Reg = reg;
...
struct ExprRes * doMult(struct ExprRes * Res1, struct ExprRes * Res2) {
	...
	AppendSeq(Res1->Instrs, Res2->Instrs);
	AppendSeq(Res1->Instrs,
			GenInstr(NULL, "mul", TmpRegName(reg), TmpRegName(Res1->Reg),
					TmpRegName(Res2->Reg)));
	ReleaseTmpReg(Res1->Reg);
	ReleaseTmpReg(Res2->Reg);
	Res1->Reg = reg;
...
ExprRes * doUOp (char * name, ExprRes * Expr){

	InstrSeq *code;

	char * newName = malloc(sizeof(char) * strlen(name));
	memcpy(newName, &name[1], strlen(name));
	printf("|%s|", newName);
}
...
struct InstrSeq * doPrint(struct ExprRes * Expr) {
	...
	AppendSeq(code,
			GenInstr(NULL, "move", "$a0", TmpRegName(Expr->Reg), NULL ));
...
extern struct BExprRes * doBExpr(struct ExprRes * Res1, struct ExprRes * Res2) {
	...
	bRes->Label = GenLabel();
	AppendSeq(Res1->Instrs,
			GenInstr(NULL, "bne", TmpRegName(Res1->Reg), TmpRegName(Res2->Reg),
					bRes->Label));
...
void Finish(struct InstrSeq *Code) {
	...
	while (entry) {
		AppendSeq(code,
				GenInstr((char *) GetName(entry), ".word", "0", NULL, NULL ));
